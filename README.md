...
```
[/requirements-breakdown →] /requirements-discuss → /requirements-review → /arch-design → /arch-review → /dev-plan → /dev-plan-review → /dev-execute
```

`/requirements-breakdown` 可选：先产出简洁的需求骨架供宏观审查，再由 `/requirements-discuss` 基于骨架逐条深化。跳过它则 `/requirements-discuss` 会从零开始探讨。

## Skill 列表

| Skill | 说明 | 产出文档 |
|-------|------|----------|
| `/requirements-breakdown` | 快速拆解项目想法为简洁的需求条目清单（名称+一句话+优先级），供宏观审查 | `project_plan/requirements-outline.md` |
| `/requirements-discuss` | 与用户多轮对话探讨需求，逐步明确功能点和验收标准；检测到 outline 时进入骨架深化模式，按条目逐条补齐多状态期望行为 | `project_plan/requirements.md` |
...
| `/quick-feature [功能描述]` | 给已有项目快速加小功能，跳过需求/架构/开发计划文档，但保留测试覆盖、安全门控、分支管理、learnings 反思 | 加一个端点/命令/选项、小改进、单上下文可完成的 S/M 规模功能 |
...
- **需求两段式**：先用 `/requirements-breakdown` 出骨架（条目+优先级），再用 `/requirements-discuss` 按条目深化，避免一次性陷入细节
